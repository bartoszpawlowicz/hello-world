from vsearch.utils.distances_utils import compute_eer, return_distances_as_array
import time
import numpy as np
import io
...
def all_distances_np(validation_set, model, max_examples, name, epoch, writer, squared):

    [x, y] = validation_set
    embeddings = model.predict(x)

    p_distances, n_distances, a_p_ids, a_n_ids = return_distances_as_array(y, embeddings, squared=squared)
...
class CheckAllDistancesCheckpoint(keras.callbacks.Callback):
    def __init__(self, valid_pickle_path, tensorboard, period=5, draw_plot=False, distance_threshold=None,
                 auto_distance_threshold=True, max_examples=10, squared=False, last_epoch=None):
        super().__init__()
    ...
    def main_function(self, epoch, max_examples, logs_dict=None):
        if logs_dict is None:
            logs_dict = {}
        start = time.time()
        name = "valid"
        with open(self.valid_pickle_path, 'rb') as f:
            print(self.valid_pickle_path)
            validation_set = pickle.load(f)

        eer, thresh, auc_score, positive_distances, negative_distances =\
            all_distances_np(
                validation_set, self.model,
                max_examples, name, epoch, self.tensorboard.writer, squared=self.squared)
        ...
        # logs_dict["accuracy_" + str(name) + "/accuracy_at_eer_thr"] = 1.-float(eer)
        # logs_dict["accuracy_" + str(name) + "/eer"] = float(eer)
        # logs_dict["accuracy_" + str(name) + "/auc"] = float(auc_score)
        ...
        # logs_dict["distances_" + str(name) + "/positive_mean_distance"] = float(pos_mean)
        # logs_dict["distances_" + str(name) + "/negative_mean_distance"] = float(neg_mean)
        # logs_dict["distances_" + str(name) + "/mean_distance_difference"] = float(diff_mean)
    ...
    def on_epoch_end(self, epoch, logs=None):
        # super(CheckAllDistancesCheckpoint, self).on_epoch_begin(epoch, logs)
        if epoch == self.last_epoch:
            logs.update(self.main_function(epoch, max_examples=self.max_examples))
            return
        if self.period > 0:
            if epoch % self.period != 0:
                return
            logs.update(self.main_function(epoch, max_examples=0))
